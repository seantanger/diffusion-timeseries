...
from models import spiking_unet_2d
from torch.utils.data import Dataset, DataLoader

class DiffusionModel_2D:
    def __init__(self, sequence_length, device, n_steps=1000, beta_start=1e-4, beta_end=0.02):
        ...
        self.model = spiking_unet_2d.Spk_UNet(T=n_steps, ch=128, ch_mult=[1, 2, 4, 8], attn=[8],num_res_blocks=2, dropout=0.1, timestep=4, img_ch=1).to(device)
        # self.model.load_state_dict(torch.load("./models/spiking_unet_noattention_128"))

    def cosine_schedule(self, n_steps, s=0.008):
    ...
    def sample(self, n_samples, batch_size, device):
        ...
        with torch.no_grad():
            for i in range(n_batches):
                ...
                x = torch.randn(current_batch_size, 1, self.sequence_length, self.sequence_length, device=device)
...
def train_2d_spiking_diffusion_model(dataset, n_epochs, lr, batch_size, device):
    # Create dataset
    sequence_length = 31
    ...
    # Initialize model
    diffusion = DiffusionModel_2D(n_steps=1000, sequence_length=sequence_length+1, device=device)
