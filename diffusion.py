...
class SpikingDiffusionModel:
    def __init__(self, sequence_length, device, n_steps=1000, beta_start=1e-4, beta_end=0.02):
        ...
        self.model = spiking_time_unet.Spk_UNet(T=n_steps, ch=128, ch_mult=[1, 2, 4, 8], attn=[8],num_res_blocks=2, dropout=0.1, timestep=8).to(device)
...
class DiffusionModel:
    def __init__(self, sequence_length, device, n_steps=1000, beta_start=1e-4, beta_end=0.02):
        ...
        self.model = timeseries_unet.UNet(T=n_steps, ch=128, ch_mult=[1, 2, 4, 8], attn=[8], num_res_blocks=2, dropout=0.1).to(device)
...
# class SpikingDiffusionModel:
#     def __init__(self, sequence_length, device, n_steps=1000, beta_start=1e-4, beta_end=0.02, schedule='linear'):
#         self.sequence_length = sequence_length
#         self.n_steps = n_steps
#         self.beta_start = beta_start
#         self.beta_end = beta_end
...
#             self.betas = torch.linspace(beta_start, beta_end, n_steps).to(device)
#         else:
#             self.betas = self.cosine_schedule(n_steps).to(device)
...
#         # Calculate loss
#         loss = nn.MSELoss(reduction='none')(predicted_noise, noise)
...
#         return total_loss.item(), loss.detach().cpu().numpy(), t.detach().cpu().numpy()
...
# def train_diffusion_model(dataset, n_epochs, lr, batch_size, device, spiking, schedule):
#     # Create dataset
#     sequence_length = 127
#     # dataset = GBMDataset(10000, sequence_length)
#     dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
...
#     # Initialize model
#     if spiking:
#         diffusion = SpikingDiffusionModel(n_steps=1000, sequence_length=sequence_length+1, device=device, schedule=schedule)
#     else:
#         diffusion = DiffusionModel(n_steps=1000, sequence_length=sequence_length+1, device=device, schedule=schedule)
#     diffusion.model.to(device)
#     optimizer = torch.optim.Adam(diffusion.model.parameters(), lr=lr)

#     # Track losses
#     losses = []
#     timestep_losses = {t: [] for t in range(diffusion.n_steps)}
#     # Training loop
#     for epoch in range(n_epochs):
#         total_loss = 0
#         for batch in dataloader:
#             batch = batch.to(device)
#             loss, batch_losses, batch_timesteps = diffusion.train_step(batch, optimizer)
#             total_loss += loss
#             losses.append(loss)
#             for t, l in zip(batch_timesteps, batch_losses):
#                 timestep_losses[t].append(l)
        
#         if (epoch + 1) % 10 == 0:
#             print(f"Epoch {epoch+1}, Average Loss: {total_loss/len(dataloader):.4f}")

#     avg_timestep_losses = {t: np.mean(losses) for t, losses in timestep_losses.items()}
#     return diffusion, losses, avg_timestep_losses
