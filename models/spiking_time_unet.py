...
class Spike_SelfAttention(nn.Module):
...
class Spk_DownSample(nn.Module):
    def __init__(self, in_ch):
        super().__init__()
        self.conv = nn.Conv1d(in_ch, in_ch, 3, stride=2, padding=1)
    ...
    def forward(self, x, temb):
        T, B, C, L = x.shape
        x = self.neuron(x)
        x = x.flatten(0, 1)  ## [T*B C L]
        x = self.conv(x)
        _, C, L = x.shape
        x = self.bn(x).reshape(T, B, C, L).contiguous()
...
class Spk_UpSample(nn.Module):
    def __init__(self, in_ch):
        super().__init__()
        self.conv = nn.Conv1d(in_ch, in_ch, 3, stride=1, padding=1)
        self.bn = nn.BatchNorm1d(in_ch)
        self.neuron = IFNode(surrogate_function=surrogate.ATan())
        functional.set_step_mode(self, step_mode='m')
        # functional.set_backend(self, backend='cupy')
...
class Spk_ResBlock(nn.Module):
    def __init__(self, in_ch, out_ch, tdim, dropout, attn=False):
        ...
        self.conv1 = nn.Conv1d(in_ch, out_ch, 3, stride=1, padding=1)
    ...
    def forward(self, x, temb):
        ...
        h = self.neuron1(x)
        h = h.flatten(0, 1)  ## [T*B C L]
        h = self.conv1(h)
        h = self.bn1(h).reshape(T, B, -1, L).contiguous()

        temp = self.temb_proj(temb).unsqueeze(-1).repeat(1, 1, 1, h.shape[-1])
        h = torch.add(h, temp)

        h = self.neuron2(h)
        h = h.flatten(0, 1)  ## [T*B C L]
        h = self.conv2(h)
        h = self.bn2(h).reshape(T, B, -1, L).contiguous()


        h = h + self.shortcut(x.flatten(0, 1)).reshape(T, B, -1, L).contiguous()
...
class MembraneOutputLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        coef = torch.pow(0.8, arr).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).to(x.device)
        out = torch.sum(x * coef, dim=0)
        return out


class Spk_UNet(nn.Module):
    def __init__(self, T, ch, ch_mult, attn, num_res_blocks, dropout, timestep, img_ch=1):
        super().__init__()
        ...
        self.conv = nn.Conv1d(img_ch, ch, kernel_size=3, stride=1, padding=1)
        self.bn = nn.BatchNorm1d(ch)

        self.neuron = IFNode(surrogate_function=surrogate.ATan())
        self.conv_identity = nn.Conv1d(ch, ch, kernel_size=3, stride=1, padding=1)
        self.bn_identity = nn.BatchNorm1d(ch)
        ...
        self.tail_bn = nn.BatchNorm1d(now_ch)
        self.tail_swish = Swish()
        self.tail_conv = nn.Conv1d(now_ch, img_ch, kernel_size=3, stride=1, padding=1)


        self.T_output_layer = nn.Conv2d(img_ch, img_ch, kernel_size=(3,1), stride=(1,1), padding=(1,0))
        self.last_bn = nn.BatchNorm1d(img_ch)
        self.swish = Swish()
    ...
    def forward(self, x, t):
        # x = x.unsqueeze(1)
        x = x.unsqueeze(0).repeat(self.timestep, 1, 1, 1)  # [T, B, C, L]
        ...
        # Downsampling
        T, B, C, L = x.shape
        h = x.flatten(0, 1)  ## [T*B C L]
        h = self.conv(h)
        h = self.bn(h).reshape(T, B, -1, L).contiguous()
        h = self.neuron(h)

        h = h.flatten(0, 1)  ## [T*B C L]
        h = self.conv_identity(h)
        h = self.bn_identity(h).reshape(T, B, -1, L).contiguous()
        ...
        h = self.tail_bn(h)
        h = self.tail_swish(h)
        h = self.tail_conv(h).reshape(T, B, -1, L).contiguous()

        h_temp = h.permute(1, 2, 3, 0)  # [ B, C, L, T]
        h_temp = self.T_output_layer(h_temp).permute(3, 0, 1, 2)   # [ T, B, C, L]
        h_temp = self.last_bn(h_temp.flatten(0,1)).reshape(T, B, -1, L).contiguous()
        h = self.swish(h_temp) + h  # [ T, B, C, L]
